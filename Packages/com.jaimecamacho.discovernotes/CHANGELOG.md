# Changelog

All notable changes to this project will be documented in this file.

The format is based on Keep a Changelog and this project follows Semantic Versioning.

## [1.3.2] - 2026-03-20

- na
- na
- na

## [1.3.1] - 2026-03-20

- fixes
- updates
- features

## [1.3.0] - 2026-03-20

- some updates
- some fixes
- more fixes

## [1.2.7] - 2026-03-16

- Publish Auto
- connect to npm
- Sign and Update

## [1.2.6] - 2026-03-16

- Published the first verified Unity-signed scoped-registry release workflow for DiscoverNotes.
- Added a manual-sign release path that prepares metadata, waits for signed `.tgz` export, and publishes to npm.
- Confirmed Unity Package Manager updates now preserve the package signature in consumer projects.

## [1.2.5] - 2026-03-16

- Added a signed Unity 6.3 release workflow based on exported `.tgz` packages.
- Added a one-command local release script for prepare, sign, and publish steps.
- Improved release documentation for team use with scoped registries in Unity.

## [1.2.4] - 2026-03-13

- Added Unity 6.3 compatibility fixes in the editor and style provider flow.
- Added release tooling for signed UPM tarball export and scoped registry publishing.
- Synced repository documentation and package metadata for repeatable team releases.

## [1.0.0] - 2025-09-12

- Initial public release of DiscoverNotes.
- Added the `GameObjectNotes` component for visual in-editor documentation.
- Added note categories, authors, hierarchy tooltip support, and rich text rendering.
- Added structured Discover sections and VAT UV atlas support.










